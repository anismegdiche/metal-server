...
export class PlanData extends absDataProvider {
	...
	async Select(
		...
		if (!source || !Source.Sources.has(source)) throw new HttpErrorBadRequest(`${schema}: plan '${source}' is missing`)

		const sourceConfig = Source.Sources.get(source)?.SourceConfig
		const planName = sourceConfig?.database

		if (!planName) throw new HttpErrorBadRequest(`${schema}: plan '${source}' is missing`)
	...
	async ListEntities(schemaRequest: TSchemaRequestListEntities): Promise<TInternalResponse<TSchemaResponse>> {
		...
		if (!planEntities || planEntities.length === 0) throw new HttpErrorNotFound(`${schema}: No entities found`)

		const data: TDataListEntity[] = planEntities.map(
			(key: string) =>
				<TDataListEntity>{
					name: key,
