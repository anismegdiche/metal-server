...
//
const _joinCaseMap: Record<string, (dtLeft: DataTable, dtRight: DataTable, leftField: string, rightField: string) => Promise<DataTable>> = {
...
export async function Join(step: TStep, $context?: Partial<TContext>): Promise<DataTable> {
    ...
    {
        return _joinCaseMap[type]!(dtLeft, dtRight, leftField, rightField) ??
            (Helper.CaseMapNotFound(type) && dtLeft)
    }
}
