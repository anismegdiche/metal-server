...
export async function ListEntities(step: TStep, $context?: Partial<TContext>): Promise<DataTable> {
	...
	return $__schemaRequest?.schema ? await _listEntitiesSchema(step) : await _listEntitiesPlan(step, $context)
}
...
async function _listEntitiesPlan(step: TStep, _$context?: Partial<TContext>): Promise<DataTable> {
	...
	const entitiesList: TDataListEntity[] = keys(ConfigManager.Get(`plans.${step.currentPlanName}`)).map(
		(entity: string) =>
			<TDataListEntity>{
				name: entity,
