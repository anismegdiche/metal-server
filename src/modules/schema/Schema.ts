...
export class Schema {
	...
	static async #MergeData(
		...
		schemaResponseToMerge: TSchemaResponse | undefined,
	): Promise<TSchemaResponse> {
		if (!schemaResponseToMerge) return schemaResponse
		...
		if (isSchemaResponseWithData && !isSchemaResponseToMergeWithData) return schemaResponse
	...
	static async Select(
		...
		return Schema.SourceTypeCaseMap[schemaRoute.type]?.(<TSourceTypeExecuteParams>{
			...
			schemaRequest,
			CrudFunction: async () => {
				const _intResp = await SourceRegistry.Sources.get(schemaRoute.routeName)?.DataProvider.Select(<TSchemaRequestSelect>{
					...schemaRequest,
					source: schemaRoute.routeName,
					entity: schemaRoute.entity ?? schemaRequest.entity,
				})

				if (!_intResp?.Body) return _intResp
				...
				if (schemaConfig?.anonymize && Schema.IsSchemaResponse(_intResp.Body) && (await _intResp.Body.data.Count()) > 0) {
					_intResp.Body.data = await DataTableUtils.Anonymize(_intResp.Body.data, schemaConfig.anonymize)
				}
				return _intResp
			},
		})
	...
	static async Delete(
		...
		return Schema.SourceTypeCaseMap[schemaRoute.type]?.(<TSourceTypeExecuteParams>{
			...
			schemaRequest,
			CrudFunction: async () => {
				return SourceRegistry.Sources.get(schemaRoute.routeName)?.DataProvider.Delete(<TSchemaRequestDelete>{
					...schemaRequest,
					source: schemaRoute.routeName,
					entity: schemaRoute.entity ?? schemaRequest.entity,
				})
	...
	static async Update(
		...
		return Schema.SourceTypeCaseMap[schemaRoute.type]?.(<TSourceTypeExecuteParams>{
			...
			schemaRequest,
			CrudFunction: async () => {
				return SourceRegistry.Sources.get(schemaRoute.routeName)?.DataProvider.Update(<TSchemaRequestUpdate>{
					...schemaRequest,
					source: schemaRoute.routeName,
					entity: schemaRoute.entity ?? schemaRequest.entity,
				})
	...
	static async Insert(
		...
		return Schema.SourceTypeCaseMap[schemaRoute.type]?.(<TSourceTypeExecuteParams>{
			...
			schemaRequest,
			CrudFunction: async () => {
				return SourceRegistry.Sources.get(schemaRoute.routeName)?.DataProvider.Insert(<TSchemaRequestInsert>{
					...schemaRequest,
					source: schemaRoute.routeName,
					entity: schemaRoute.entity ?? schemaRequest.entity,
				})
