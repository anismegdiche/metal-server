"""
HuggingFace Document Processing Service

This module provides a FastAPI-based service for running HuggingFace document processing tasks.
"""
...
app = FastAPI(
    title="HuggingFace Document Processing Service",
    description="API for running HuggingFace document processing tasks",
    version="1.0.0",
    openapi_prefix=""
)
app.add_middleware(
...
router = APIRouter(prefix="/huggingface/document")

# Document task config (flat)
TASKS = {
...
@router.get(
    ...
    summary="List Tasks",
    description="List all available document processing tasks"
)
...
@router.post(
    ...
    description="Process a document file using the specified HuggingFace task"
)
async def run_document_task(
    ...
    try:
        ...
        start_time = time.time()
        try:
            result = pipe(doc_path, **{k: v for k, v in task_params.items() if k != 'model'})
