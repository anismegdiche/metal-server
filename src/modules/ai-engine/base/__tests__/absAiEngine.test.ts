...
class TestAiEngine extends absAiEngine {
	...
	AiDockerService = {}
	RunTask = {}
	async Run(_params: TAiArguments): Promise<TAiOutput> {
		return { status: "ok" } as TAiOutput
	}
...
describe("absAiEngine", () => {
	...
	it("should retry on non-429 errors and eventually succeed", async () => {
		...
		vi.mocked(axios.post).mockRejectedValueOnce({ message: "fail" }).mockResolvedValueOnce({ status: 200 })
