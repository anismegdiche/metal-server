...
import { TConfigAiEngineNlpJs, TConfigAiEngineNlpJsGuessLangOptions, TConfigAiEngineNlpJsSentimentOptions } from '../@types'
import { NLP_JS_MODEL, AI_ENGINE } from '../@consts'
...
export class NlpJs implements IAiEngine {
    ...
    async Run(text: string): Promise<any> {
        return await this.#RunModel[this.Model](text)
            .catch((error: any) => {
                Logger.Error(
                    `NlpJs.Run '${this.InstanceName}': '${JsonUtils.Stringify(this.Options)}',Text= '${text}'`
                )
                Logger.Error(error)
                return undefined
            })
    ...
    static #SentimentAnalyzeSetDefaultOptions(
        aiEngineConfigOptions: Partial<TConfigAiEngineNlpJsSentimentOptions>
    ): TConfigAiEngineNlpJsSentimentOptions {
        return {
            lang: 'en',
    ...
    static #GuessLanguageSetDefaultOptions(
        aiEngineConfigOptions: Partial<TConfigAiEngineNlpJsGuessLangOptions>
    ): TConfigAiEngineNlpJsGuessLangOptions {
