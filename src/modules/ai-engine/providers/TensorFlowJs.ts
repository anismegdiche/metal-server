...
import { TConfigAiEngineTensorFlowJsImageClassifyOptions } from '../@types'
import { TENSORFLOW_JS_MODEL, AI_ENGINE } from '../@consts'
...
export class TensorFlowJs implements IAiEngine {
	...
	async Run(imagePath: string): Promise<any> {
		return await this.#RunModel[this.Model](imagePath)
			.catch((error: any) => {
				Logger.Error(`TensorFlowJs.Run '${this.InstanceName}': '${JsonUtils.Stringify(this.Options)}', on '${imagePath}'`)
				Logger.Error(error)
				return undefined
			})
	}
