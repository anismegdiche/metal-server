...
import { CosmosClient, Container, Database, SqlQuerySpec, OperationInput, PartitionKeyKind, ConnectionMode, CosmosClientOptions, ContainerResponse } from "@azure/cosmos"
//
import { absDataProvider } from "../absDataProvider"
import { DATA_PROVIDER } from "../DataProvider"
import { TConfigSource } from "../../types/TConfig"
import { TSchemaRequest, TSchemaRequestDelete, TSchemaRequestInsert, TSchemaRequestUpdate, TSchemaRequestSelect, TSchemaRequestListEntities } from '../../types/TSchemaRequest'
import { TInternalResponse } from '../../types/TInternalResponse'
import { TSchemaResponse } from "../../types/TSchemaResponse"
import { HttpErrorInternalServerError, HttpErrorBadRequest, HttpErrorNotFound, HttpErrorNotImplemented } from "../../server/HttpErrors"
...
export class CosmosDbData extends absDataProvider {
    ...
    async Init(source: string, sourceConfig: TConfigSource): Promise<void> {
        ...
        Logger.Assert(!StringHelper.IsEmpty(this.Config.options.endpoint), `${Logger.Out} ${this.SourceName}: Cosmos DB endpoint is required`)
        Logger.Assert(!StringHelper.IsEmpty(this.Config.options.key), `${Logger.Out} ${this.SourceName}: Cosmos DB key is required`)
        Logger.Assert(!StringHelper.IsEmpty(this.Config.database), `${Logger.Out} ${this.SourceName}: Cosmos DB database is required`)
    }
    ...
    async Select(schemaRequest: TSchemaRequestSelect, $context?: Partial<TContext>): Promise<TInternalResponse<TSchemaResponse>> {
        Logger.Assert<Database>(this.Database, this.Database !== undefined, `${Logger.Out} ${this.SourceName}: Database not connected`)
    ...
    async GetContainer(schemaRequest: TSchemaRequest): Promise<Container> {
        ...
        try {
            ...
            const containerName = Body?.data.FilterRows(`name = "${schemaRequest.entity}"`).Rows[0] as { name: string, partitionKey: string }
            const { container } = await this.Database.containers.createIfNotExists({
                id: containerName.name,
                partitionKey: containerName.partitionKey
            })
