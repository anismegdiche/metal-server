...
import { AI_ENGINE, NLP_JS_MODEL, TConfigAiEngineNlpJs, TConfigAiEngineNlpJsGuessLangOptions, TConfigAiEngineNlpJsSentimentOptions } from '../server/AiEngine'
...
export class NlpJs implements IAiEngine {
    ...
    async Run(text: string): Promise<any> {
        return await this.#RunModel[this.Model](text)
            .catch((error: any) => {
                Logger.Error(
                    `NlpJs.Run '${this.InstanceName}': '${JsonHelper.Stringify(this.Options)}',Text= '${text}'`
                )
                Logger.Error(error)
                return undefined
            })
