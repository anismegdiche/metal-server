...
export class DataBase {
	...
	constructor(name: string, isPersistant?: boolean) {
		...
		this.Name = name
		this._dbPath =
			isPersistant === true ? StringUtils.Path(DATATABLE_TEMP_PATH, `${this.Name}_${Utils.Uuid(true)}.db`) : ":memory:"
	}
	...
	async FreeSql({
		...
		if (returnData) {
			const result = new DataTable(
				this.Name,
				await this.Tables[entity]?._runSqlAndGetRows(_sql, queryParams).catch((err) => {
