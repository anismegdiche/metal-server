...
type HookAction = 'CONTINUE' | 'STOP' | 'RETRY' | 'SKIP' // STOP returns current data instead of throwing
...
class PlanProcessor {
  ...
  private async executeStep(
    ...
    for (const hook of startHooks) {
      ...
      if (action === 'STOP') return context.$plan.$current.data // Stop execution gracefully
    }
