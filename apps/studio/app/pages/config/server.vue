    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold">
          <UIcon name="i-lucide-sliders-horizontal" class="ml-0 mr-2" />Server Configuration
        </h1>
        <p class="text-sm text-muted">Configure server settings from metal.yaml</p>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <UCard class="bg-metal-gradient">
        ...
        <div class="flex flex-col gap-4">
          <!---->
          <UFormField label="Port" description="Server TCP port" size="md" orientation="horizontal">
      ...
      <UCard class="bg-metal-gradient">
        ...
        <div class="flex flex-col gap-4">
          ...
          <template v-if="authProvider === 'oidc'">
            ...
            <UFormField label="Roles Path" description="JSON path to roles in token (optional)" orientation="horizontal">
      ...
      <UCard class="bg-metal-gradient">
        ...
        <div class="flex flex-col gap-4">
          <UFormField label="Enabled" description="Enable cache backend" orientation="horizontal">
            <USwitch v-model="cacheEnabled" />
          </UFormField>
          <template v-if="cacheEnabled">
            <UFormField label="Provider" description="Cache storage provider" orientation="horizontal">
              <USelect v-model="cacheProvider" :items="cacheProviderItems" />
            </UFormField>
            <UFormField label="Host" description="Cache server hostname" orientation="horizontal">
              <UInput v-model="cacheHost" placeholder="localhost" />
            </UFormField>
            <UFormField label="Port" description="Cache server port" orientation="horizontal">
              <UInput v-model="cachePort" type="number" />
            </UFormField>
            <UFormField label="Database" description="Database name" orientation="horizontal">
              <UInput v-model="cacheDatabase" placeholder="metal_cache" />
            </UFormField>
            <UFormField label="User" description="Database user" orientation="horizontal">
              <UInput v-model="cacheUser" placeholder="postgres" />
            </UFormField>
            <UFormField label="Password" description="Database password" orientation="horizontal">
      ...
      <UCard class="bg-metal-gradient lg:col-span-2">
        ...
        <div class="flex flex-col gap-4">
          <UFormField label="Enabled" description="Enable AI engine support" orientation="horizontal">
          ...
          <template v-if="aiEnginesEnabled">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div class="flex flex-col gap-4">
                ...
                <UFormField label="Sleep (ms)" description="Sleep between health checks (5000-600000)" orientation="horizontal">
              ...
              <div class="flex flex-col gap-4">
                ...
                <UFormField label="CPU Scale Up (%)" description="CPU threshold to scale up (10-100)" orientation="horizontal">
                  <UInput v-model="aiEnginesCpuScaleUp" type="number" :min="10" :max="100" />
                </UFormField>
                <UFormField label="CPU Scale Down (%)" description="CPU threshold to scale down (0-50)" orientation="horizontal">
                  <UInput v-model="aiEnginesCpuScaleDown" type="number" :min="0" :max="50" />
                </UFormField>
                <UFormField label="Scale Interval (ms)" description="Check interval (5000-600000)" orientation="horizontal">
                  <UInput v-model="aiEnginesScaleInterval" type="number" />
                </UFormField>
                <UFormField label="Scale Down Grace (ms)" description="Grace period before scaling down (5000-600000)" orientation="horizontal">
