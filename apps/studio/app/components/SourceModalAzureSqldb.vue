...
function collectBody() {
  const body: Record<string, any> = { provider: 'azure-sqldb' }
  ...
  <div class="flex flex-col gap-4">
    <div class="grid grid-cols-2 gap-4">
      <UFormField label="Host" orientation="horizontal" :ui="{ description: 'text-xs' }" description="Server to connect to">
        <UInput v-model="form.host" placeholder="myserver.database.windows.net" class="w-full" />
      </UFormField>
      ...
      <UFormField label="User" orientation="horizontal" :ui="{ description: 'text-xs' }" description="User name for authentication">
        <UInput v-model="form.user" placeholder="myuser" class="w-full" />
      </UFormField>
    ...
    <div class="border-t border-default pt-4">
      ...
      <div v-if="optionsOpen" class="mt-3 grid grid-cols-1 gap-4">
        <UFormField label="Domain" orientation="horizontal" :ui="{ description: 'text-xs' }" description="Domain for domain login to Azure SQL">
        ...
        <UFormField hidden label="Encrypt" orientation="horizontal" :ui="{ description: 'text-xs' }" description="Encrypt connection (enabled by default for Azure)">
