...
export class ApiKey {
	...
	static _store: PersistentMap<TApiKeyRecord> | undefined = undefined

	static get #store(): PersistentMap<TApiKeyRecord> {
		if (!ApiKey._store) {
			ApiKey._store = new PersistentMap<TApiKeyRecord>(Env.server.apiKeys.path)
		}
		return ApiKey._store
	}
