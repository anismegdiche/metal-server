...
import { AzureCosmosDbData } from "../providers/AzureCosmosDbData"

describe("AzureCosmosDbData", () => {
	describe("GenerateCountQuery", () => {
		it("should generate an unfiltered count query in Cosmos syntax", () => {
			const provider = new AzureCosmosDbData()
			const schemaRequest = <TSchemaRequest>{ schema: "s", entity: "e" }
			const options = <TOptionalParameter>{}

			const query = provider.GenerateCountQuery(schemaRequest, options)
