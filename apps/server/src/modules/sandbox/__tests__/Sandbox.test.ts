...
describe("Sandbox", () => {
	...
	describe("Malicious Code Tests", () => {
		...
		it("should reject code attempting to execute shell commands (exec)", () => {
			expect(() => sandbox.Evaluate('require("child_process").execSync("rm -rf /")')).toThrow(HttpErrorInternalServerError)
		})

		it("should reject code attempting to access the file system", () => {
			expect(() =>
				sandbox.Evaluate('const fs = require("fs"); fs.writeFileSync("malicious.txt", "Malicious content")'),
			).toThrow(HttpErrorInternalServerError)
		})

		it("should reject code attempting to make network requests", () => {
			expect(() => sandbox.Evaluate('const http = require("http"); http.get("http://malicious-site.com")')).toThrow(
				HttpErrorInternalServerError,
			)
		})

		it("should reject code attempting to modify global objects", () => {
			expect(() => sandbox.Evaluate('Object.prototype.maliciousFunction = () => console.log("Malicious")')).toThrow(
				HttpErrorInternalServerError,
			)
		})

		it("should reject code attempting to access sensitive environment variables", () => {
			expect(() => sandbox.Evaluate("console.log(process.env.SENSITIVE_DATA)")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject code attempting to spawn child processes", () => {
			expect(() => sandbox.Evaluate('require("child_process").fork("maliciousScript.js")')).toThrow(
				HttpErrorInternalServerError,
			)
		})

		it("should reject code attempting to use eval", () => {
			expect(() => sandbox.Evaluate("eval(\"console.log('Eval is dangerous!')\")")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject code attempting to access internal modules", () => {
			expect(() => sandbox.Evaluate('const internalModule = require("internal-module")')).toThrow(
				HttpErrorInternalServerError,
			)
		})
		...
		it("should reject code attempting to manipulate the prototype chain", () => {
			expect(() =>
				sandbox.Evaluate('Array.prototype.customFunction = () => console.log("Manipulating prototype chain")'),
			).toThrow(HttpErrorInternalServerError)
		})

		it("should reject code attempting to create infinite recursion", () => {
			expect(() => sandbox.Evaluate("function infiniteRecursion() { infiniteRecursion() } infiniteRecursion()")).toThrow(
				HttpErrorInternalServerError,
			)
		})

		it("should reject code attempting to access restricted APIs", () => {
			expect(() => sandbox.Evaluate('const crypto = require("crypto")')).toThrow(HttpErrorInternalServerError)
		})

		it("should reject code that executes shell commands (exec)", () => {
			expect(() => sandbox.Evaluate("require('child_process').exec('ls')")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject code that exits the process", () => {
			expect(() => sandbox.Evaluate("process.exit(1)")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject code that uses eval", () => {
			expect(() => sandbox.Evaluate("eval('2 + 2')")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject code that manipulates object prototypes", () => {
			expect(() => sandbox.Evaluate("Object.prototype.hacked = true")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject infinite loops", () => {
			expect(() => sandbox.Evaluate("while(true) {}")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject code that makes network requests", () => {
			expect(() => sandbox.Evaluate("fetch('http://malicious.com')")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject function declarations", () => {
			expect(() => sandbox.Evaluate("function test() { return 'hacked' }")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject setTimeout and setInterval usage", () => {
			expect(() => sandbox.Evaluate("setTimeout(() => console.log('hacked'), 1000)")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject command injection via concatenation", () => {
			expect(() => sandbox.Evaluate("const cmd = 'ls'; require('child_process').exec(cmd)")).toThrow(
				HttpErrorInternalServerError,
			)
		})

		it("should reject environment variable access", () => {
			expect(() => sandbox.Evaluate("process.env.SECRET")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject accessing global objects", () => {
			expect(() => sandbox.Evaluate("global.process.exit(1)")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject modifying the global object", () => {
			expect(() => sandbox.Evaluate("global.hacked = true")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject accessing the Function constructor", () => {
			expect(() => sandbox.Evaluate("Function('return process')()")).toThrow(HttpErrorInternalServerError)
		})

		it("should reject this.constructor.constructor('return process')().exit()", () => {
			expect(() => sandbox.Evaluate("this.constructor.constructor('return process')().exit()")).toThrow(
				HttpErrorInternalServerError,
			)
		})

		it("should reject process.exit()", () => {
			expect(() => sandbox.Evaluate("process.exit()")).toThrow(HttpErrorInternalServerError)
		})
	})
})
