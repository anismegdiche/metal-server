...
type ProviderMap = {
	[key in STORAGE_TYPE]: ProviderLoader
}

export class StorageProvider {
	static readonly #storageFactory = new Factory<Promise<IStorageProvider>>()
	static readonly #loadingPromises = new Map<STORAGE_TYPE, Promise<IStorageProvider>>()
	static readonly #providerMap: ProviderMap = {
		[STORAGE_TYPE.FILESYSTEM]: () => import("./providers/FsStorage").then((m) => m.FsStorage),
		[STORAGE_TYPE.FTP]: () => import("./providers/FtpStorage").then((m) => m.FtpStorage),
		[STORAGE_TYPE.SFTP]: () => import("./providers/SftpStorage").then((m) => m.SftpStorage),
		[STORAGE_TYPE.AZURE_BLOB]: () => import("./providers/AzureBlobStorage").then((m) => m.AzureBlobStorage),
		[STORAGE_TYPE.AZURE_FILE]: () => import("./providers/AzureFileStorage").then((m) => m.AzureFileStorage),
		[STORAGE_TYPE.AZURE_DATALAKE_G2]: () => import("./providers/AzureDataLakeStorage").then((m) => m.AzureDataLakeStorage),
		[STORAGE_TYPE.AWS_S3]: () => import("./providers/AmazonS3Storage").then((m) => m.AmazonS3Storage),
	}

	static async GetProvider(providerName: STORAGE_TYPE): Promise<IStorageProvider> {
