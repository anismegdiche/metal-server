...
export async function MapRows(stepParams: U__plans_plan__step_Params, $context: Partial<TContext>): Promise<DataTable> {
	...
	return planData.RowsMap((row: TRow) => _mapRow(row, stepParams, $context))
}

export async function _mapRow(
	row: TRow,
	...
	const $__stepParams = PlaceHolder.EvaluateJsCode<U__plans_plan_map_Params>(
		stepParams,
		new Sandbox($context),
	) as U__plans_plan_map_Params

	const { script } = $__stepParams
