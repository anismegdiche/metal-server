...
describe("Mutex", () => {
	...
	describe("Once", () => {
		...
		it("should handle concurrent access safely", async () => {
			...
			const promises = Array.from({ length: 10 }, () => instance.counter)
		...
		it("should work with multiple properties on same class", async () => {
			...
			const [value1, value2] = await Promise.all([instance.prop1, instance.prop2])
		...
		it("should maintain separate mutexes for different instances", async () => {
			...
			const [value1, value2] = await Promise.all([instance1.sharedValue, instance2.sharedValue])
	...
	describe("CreateMutexProtected", () => {
		...
		it("should maintain separate state for different instances", async () => {
			...
			const [value1, value2] = await Promise.all([counter1.get(), counter2.get()])
