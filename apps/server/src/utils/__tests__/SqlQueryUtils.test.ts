...
describe("SqlQueryUtils", () => {
	describe("Value Formatting", () => {
		...
		it("should handle string values with single quotes", () => {
			const sql = new SqlQueryUtils().Update("users").Set({
				id: 1,
				name: "O'Reilly",
			})
		...
		it("should handle numeric values", () => {
			const sql = new SqlQueryUtils().Update("products").Set({
				id: 1,
				price: 9.99,
				quantity: "10",
			})
		...
		it("should handle object values with parameter binding", () => {
			const date = new Date()
			const sql = new SqlQueryUtils().Update("events").Set({
				id: 1,
				data: {
					date,
					type: "test",
	...
	describe("INSERT operations", () => {
		it("should handle multiple rows with values", () => {
			...
			expect(sql.QueryParams).toEqual([1, "John", true, 2, "Jane", false])
		})
	...
	describe("Sql injection test", () => {
		...
		it("should throw error for deny characters inside string and deny words", () => {
			const queryHelper = new SqlQueryUtils(
				"SELECT * FROM users WHERE name = 'John; | AND OR | DROP TABLE users'; SELECT * FROM users;;; -- Comments",
			)
			expect(() => queryHelper.Query()).toThrow(HttpErrorBadRequest)
		})
		...
		it("should throw for pipe concatenation with subquery", () => {
			const queryHelper = new SqlQueryUtils(
				`SELECT * FROM users WHERE name = "' || (SELECT password FROM secrets WHERE id=1) || '"`,
			)
			expect(() => queryHelper.Query()).toThrow(HttpErrorBadRequest)
		})
		...
		it("should throw for percent-encoded UNION", () => {
			const queryHelper = new SqlQueryUtils(
				`SELECT * FROM users WHERE name = "'%75%6e%69%6f%6e%20select%20%2a%20from%20users%20--"`,
			)
			expect(() => queryHelper.Query()).toThrow(HttpErrorBadRequest)
		})
		...
		it("should throw for stacked UPDATE after terminator", () => {
			const queryHelper = new SqlQueryUtils(
				`SELECT * FROM users WHERE name = "'; UPDATE users SET admin=1 WHERE id=1; --"`,
			)
			expect(() => queryHelper.Query()).toThrow(HttpErrorBadRequest)
		})
		...
		it("should throw for INSERT into admin_log attempt", () => {
			const queryHelper = new SqlQueryUtils(
				`SELECT * FROM users WHERE name = "'; INSERT INTO admin_log (msg) VALUES ('hacked'); --"`,
			)
			expect(() => queryHelper.Query()).toThrow(HttpErrorBadRequest)
		})
